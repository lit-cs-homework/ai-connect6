...
import static stud.g02.Road._FORWARD;
...
public class RoadTable {
		...
		for(int i = 0; i < 6; i++)
			for(int k = 0; k < 4; k++ ){
				...
				if(index>=0 && index <SIDE*SIDE){
					if (roads[index][k].isiflegal() == true)
	...
	//获取有四个黑子的路
	public RoadSet get_four_roads_black() {
		RoadSet temp = new RoadSet();
		temp = playerRoads[4][0];
		return temp;
	}
	//获取有四个白子的路
	public RoadSet get_four_roads_white() {
		RoadSet temp = new RoadSet();
		temp = playerRoads[0][4];
		return temp;
	}
	...
	/**
	 *  evaluateChessScore方法用来评估棋局的局势信息，根据六子棋博弈的评估函数论文编写。
	 *  @param pieceColor 要评估局势的一方所持棋子颜色
	 *  @param roadTable 当前的棋局路表
	 *  @return current 当前棋子颜色对应一方棋局的评估分数
	 */
	public static int evaluateChessScore(PieceColor pieceColor,
										  RoadTable roadTable) {
		int currentScore = 0;
