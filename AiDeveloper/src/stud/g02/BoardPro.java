...
public class BoardPro extends Board {
	...
	//计算迫着，color受到的威胁数(堵住所有四子五子至少要下几颗棋)
	int countAllThreats(PieceColor color) {
		...
		//是否是单威胁 ，就看是否有一个子把所有威胁接触
		RoadSet roadList = (four.size() == 0 ? five : four);
	...
	void findblanks(){
		for(int i = 0; i < SIDE*SIDE; i++){
			if(battle[i] > 0 && get(i)== EMPTY){
				changeRoads(i);
				int score = RoadTable.evaluateChessScore(whoseMove(), roadTable);
				unchangeRoads(i);
	...
	public ArrayList<MovePro> findGenerateMoves() {
		...
		for(Node node : pointslist){
			for(int i = ++index; i < choose_num; i++){
				...
				changeRoads(move);
				move.setScore(RoadTable.evaluateChessScore(whoseMove(), roadTable));
				unchangeRoads(move);
				moves.add(move);
			}
			choose_num--;
